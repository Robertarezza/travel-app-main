...
export default {
  ...
  methods: {
    initializeMap() {
      ...
      config.apiKey = "uE0fYaTLge27b5iJRggg";
      config.primaryLanguage = maptilersdk.Language.ITALIAN;
      const initialState = { lng: 12.56738, lat: 41.871941, zoom: 5.2 };
      ...
      try {
        const response = await axios.get(
          `https://api.tomtom.com/search/2/geocode/${encodeURIComponent(
            address
          )}.json?key=ZJGdhAIHS1ggLuLLDQG2OQFu9b6H3q9w`
        );
    ...
    addMarker(lat, lon, title, description, image, index) {
      if (this.map) {
        const marker = new Marker({ color: "#359cd8" })
          .setLngLat([lon, lat])
          .addTo(this.map);
...
<template>
  ...
  <div class="map-wrap">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h1 class="modal-title fs-5" id="addLocationModalLabel">
          ...
          <div class="modal-body">
                <form>
                    ...
                    <textarea id="description" class="form-control" maxlength="500" rows="3" v-model="form.description" autocomplete="off"></textarea>
                    <label for="image">Immagine</label>
                    <input type="file" @change="handleImageUpload" class="form-control" id="image"/>
                
                </form>
                <div class="vote-container mt-2">
                    <i class="fa-solid fa-star " v-for="(star, index) in 5" :key="index" @click="saveIndex(index)"
                  :class="{ 'active-star': index < form.index }"></i>
            ...
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
              Chiudi</button>
            <button type="submit" class="btn btn-primary" @click.prevent="saveForm" data-bs-dismiss="modal">
              Salva
            </button>
            </div>
        </div>
    ...
    <div class="d-flex" style="height: 14%; align-items: center">
      <button
        type="button"
        class="btn btn-default btn-circle btn-lg ms-btn"
      ...
      <p
        class="scritta"
        style="margin-left: 5px; margin-top: 4px; margin-bottom: 0 !important"
      >
        Aggiungi Luogo
      </p>
    </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <input
              type="file"
              @change="handleMarkerImageUpload"
              class="form-control"
              id="markerImage"
            />
            ...
            <div class="vote-container mt-2">
              <i
                ...
                :class="{ 'active-star': index < selectedMarker.index }"
              ></i>
            </div>
          </div>
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-danger" @click="deleteMarker">
              Elimina
            </button>
          </div>
        </div>
